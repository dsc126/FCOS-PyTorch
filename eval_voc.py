...
def acc_2d(cubes_a, cubes_b, img_shape):
    """
    numpy 计算Acc
    :param cubes_a: [N,(x1,y1,x2,y2)]
    :param cubes_b: [M,(x1,y1,x2,y2)]
    ...
    # compute acc
    acc =  1 + (2*overlap-area_a-area_b)/img_shape[0]/img_shape[1]
    return acc
...
def eval_ap_2d(gt_boxes, gt_labels, pred_boxes, pred_labels, pred_scores, iou_thread, num_cls, image_shapes):
    """
    ...
    for label in range(num_cls)[1:]:
        ...
        # image_shape_single_cls = [image_shape[mask] for image_shape, mask in zip(image_shapes, true_label_loc)]
        ...
        ious = np.zeros((0,))
        accs = np.zeros((0,))
        for sample_gts, sample_pred_box, sample_scores, image_shape in zip(gt_single_cls, bbox_single_cls, scores_single_cls, image_shapes):
            ...
            for index in range(len(sample_pred_box)):
                ...
                acc = acc_2d(sample_gts, pred_box, image_shape)
                accs = np.append(accs, acc[np.argmax(acc, axis=0),0])
        ...
        all_ap[label] = ap
        all_iou[label] = non_zero_mean(ious)
        all_acc[label] = non_zero_mean(accs)
        # print(recall, precision)
    return all_ap, all_iou, all_acc

# def eval_acc(gt_labels, pred_labels, num_cls):
#     acc = 0
#     for gt_label, pred_label in zip(gt_labels, pred_labels):
#         for g_l in gt_label:
#             acc += sum([p_l==i for pred_label])/len(pred_label)
#     acc = acc/len(pred_labels)
    

def _fast_hist(label_true, label_pred, n_class):
    # calculate confusion matrix based on true labels and prediction labels
    mask = (label_true >=0) & (label_true<n_class)
    print(mask, label_true[mask])
    hist = np.bincount(
        n_class*label_true[mask].astype(int) + label_pred[mask], 
        minlength=n_class**2).reshape(n_class, n_class)

def label_accuracy_score(label_trues, label_preds, n_class):
    print(label_trues[0:3], label_preds[0:3])
...
if __name__=="__main__":
    ...
    eval_dataset = VOCDataset(root_dir='data/VOCdevkit/VOC0712', resize_size=[800, 1333],
